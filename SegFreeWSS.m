function SegFreeWSS(sourceDir,outDir,expt,dimT,numFiles,parallelitySwitch,parallelityCutoff,onlyVel)
%%
% Volumetric Segmentation-Free Method for Rapid Visualization of Vascular Wall Shear Stress Using 4D Flow MRI 
% AUTHORS: Evan M Masutani1,2, Francisco Contijoch PhD3,4, Espoir Kyubwa PhD1,2, Joseph Cheng PhD5, Marcus T Alley PhD5, Shreyas Vasanawala MD PhD5, Albert Hsiao MD PhD2,4*
% Institutions: 1Medical Scientist Training Program, 2Department of Medicine, 3Department of Bioengineering, and 4Department of Radiology, University of California, San Diego, La Jolla, CA 92092, USA
% 5Department of Radiology, Stanford University, Stanford, CA 94305, USA

%% EXPLANATION OF INPUTS
% sourceDir: str name of source directory for 4D flow input data
% outDir: str name of output directory
% expt: str identifier inserted into DICOM metadata to keep track
% dimT: str number of timepoints in scan
% numFiles: str total number of frames for scan (number of slices * dimT)
% parallelitySwitch: str 0 or 1, where 0 turns off parallelity and 1 turns it on
% parallelity: option to filter out luminal elements; works by solving for the 
% dot product of grad(speed) and the normal vector and setting WSS of all elements
% less than the value of parallelityCutoff to zero
% parallelityCutoff: str value of angle cutoff to remove luminal elements.  
% Published data assumed cutoff of 0 (removes any antiparallel elements)
% onlyVel: str value -1 or 1; published data uses a value of 1.  If value < 0, 
% apply parallelity thresholding to edge intensity data as well; else, only
% apply thresholding to WSS values.
...
parfor t = startT:1:dimT
    ...
    %% PART 4: CALCULATE GRAD INTENSITY (positive grad, inward normal since lumen is bright)
    %d/dx
